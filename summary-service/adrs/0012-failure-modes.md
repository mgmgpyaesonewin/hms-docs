...
Exhaustive enumeration of failure modes. The response for each must be specified in [[diagrams/sequences|diagrams/sequences.md]] and tested in the runbook ([[ops/runbook|ops/runbook.md]]).

## Failure modes and responses

| # | Failure | Response |
|---|---|---|
| 1 | Worker crashes mid-event-processing — DB commit succeeded, Redis update not yet done | DB row is committed (CFI exists with the event's data); outbox row is NOT marked processed (the transaction is `INSERT cfi; UPDATE outbox SET status='DONE', completed_at=now()`; if the worker dies between the two, the outbox row stays unprocessed). On next poll, the event is reprocessed. The `event_id` unique constraint on CFI causes the re-insert to be a no-op; the outbox row is then marked processed. Redis update runs at the end and is safe under at-least-once (see [[0009-redis-cache-model|ADR 0009]]). Net effect: no data loss, possibly one extra Redis write. |
...
| 4 | Redis is down during a read | The API falls back to computing the counter from Postgres (see [[0009-redis-cache-model|ADR 0009]]). Response includes a `X-Cache-Status: bypass` header. |
...
- The API has explicit `try / catch` blocks for Redis errors that fall back to Postgres (per [[0009-redis-cache-model|ADR 0009]]).
- The runbook ([[ops/runbook|ops/runbook.md]]) covers each of these failure modes with a "what to do" procedure.
...
| # | Failure | Response |
|---|---|---|
| 6 | Status update requested on a CFI from a different tenant | API returns `404 Not Found` (not `403 Forbidden` — don't leak the existence of a cross-tenant row). Defense comes from the `tenantId` on every query (Prisma extension, [[0007-multi-tenancy-enforcement|ADR 0007]]) and the per-tenant Redis key prefix. This case should be unreachable in practice — v1 has no auth, so trust in the `X-Tenant-Id` header is by client agreement; v2 will bind the tenant to the auth context. |
...
- [[0009-redis-cache-model|ADR 0009]] (Redis cache model — covers failure modes 3, 4)
- [[0001-trigger-mechanism|ADR 0001]] (Outbox — covers failure mode 8)
