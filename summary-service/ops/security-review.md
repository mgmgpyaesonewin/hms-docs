...
|---|---|---|
| **Spoofing** | A non-BFF process calls the Summary Service API and pretends to be the BFF. | The service binds to `127.0.0.1` only. v1 has no auth; any local process can call the API. The threat is bounded by who has shell access on the host. (v2 will add a real service-to-service auth.) |
...
| **Information disclosure** | Logs leak sensitive data. | `pino` `redact` paths exclude `authorization` and any `hmacSecret` field. No patient name, doctor name, or amount is logged at `info` level. |
| **Information disclosure** | Disk is stolen (theft of the on-prem server). | LUKS full-disk encryption is the hospital's existing posture; the Summary Service does not weaken it. No HMAC secret on disk in v1. |
| **Denial of service** | Attacker floods the API with requests. | The API binds to `127.0.0.1`; the only caller is the BFF. No external DoS surface. Internal DoS (a buggy BFF) is mitigated by request timeout (10s). |
...
| **Elevation of privilege** | (no HMAC secret in v1 to read) | n/a |
...
**Capability:** an attacker has compromised the HMS Next.js BFF process and can call the Summary Service (no auth in v1).
...
**Mitigation:** service-to-service auth is a v2 feature. For v1, the BFF compromise is a high-impact event that requires BFF-level security hardening (out of scope for this review).
...
|---|---|---|---|
| (none) | _v1 has no service-to-service auth, so no shared secret is stored on disk._ | _n/a_ | _n/a_ |
...
- **Service-to-service auth.** v1 has none. v2 should add a real auth mechanism (TBD — HMAC, mTLS, or short-lived service JWTs are the candidates).
